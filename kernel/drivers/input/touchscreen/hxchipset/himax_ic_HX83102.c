...
static int hx83102ab_read_FW_status(uint8_t *state_addr, uint8_t *tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		...
		for (i = 0; i < DATA_LEN_4; i++) {
			/* 0x10007F44 */
			state_addr[i + 2] =
				(*kp_pfw_op)->addr_fw_dbg_msg_addr[i];
			status_addr[i] = (*kp_pfw_op)->addr_fw_dbg_msg_addr[i];
		}

		req_size = 0x04;
		kp_g_core_fp->fp_sense_off(true);
		kp_g_core_fp->fp_register_read(status_addr, req_size,
			tmp_addr, false);
		kp_g_core_fp->fp_sense_on(0x01);
		...
		for (i = 0; i < DATA_LEN_4; i++) {
			/* 0x900000F8 */
			state_addr[i + 2] =
				(*kp_pfw_op)->addr_fw_state_addr[i];
			cmd_addr[i] = (*kp_pfw_op)->addr_fw_state_addr[i];
		}

		req_size = 0x30;
		kp_g_core_fp->fp_register_read(cmd_addr, req_size,
			tmp_addr, false);
	}
...
static int hx83102d_0f_overlay(int ovl_type, int mode)
{
	...
	char fwname[32] = "Himax_firmware.bin";
	uint8_t part_num = 0;
	uint8_t buf[16];
	...
	ret = request_firmware(&fwp, fwname, (*kp_private_ts)->dev);
	if (ret < 0) {
 ...
 */
	...
	/* rescue mechanism */
	if (count >= 10) {
		kp_g_core_fp->fp_0f_op_file_dirly(fwname);
...
static void himax_hx83102ab_reg_re_init(uint8_t ic_name)
{
	...
	if (ic_name == 0x2a) {
		...
		kp_himax_parse_assign_cmd(hx83102a_data_df_x_res,
				(*kp_pdriver_op)->data_df_x_res,
				sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_parse_assign_cmd(hx83102a_data_df_y_res,
				(*kp_pdriver_op)->data_df_y_res,
				sizeof((*kp_pdriver_op)->data_df_y_res));
	} else {
		kp_himax_parse_assign_cmd(hx83102b_data_df_x_res,
				(*kp_pdriver_op)->data_df_x_res,
				sizeof((*kp_pdriver_op)->data_df_x_res));
		kp_himax_parse_assign_cmd(hx83102b_data_df_y_res,
				(*kp_pdriver_op)->data_df_y_res,
				sizeof((*kp_pdriver_op)->data_df_y_res));
	}
}
...
static void himax_hx83102ab_func_re_init(void)
{
	...
	kp_g_core_fp->fp_read_FW_status = hx83102ab_read_FW_status;
...
#if defined(HX_ESD_RECOVERY)
	kp_g_core_fp->fp_esd_ic_reset = hx83102ab_esd_ic_reset;
#endif
#if defined(HX_ZERO_FLASH)
...
#endif
	kp_himax_parse_assign_cmd(hx83102d_data_df_x_res,
			(*kp_pdriver_op)->data_df_x_res,
			sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_parse_assign_cmd(hx83102d_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
			sizeof((*kp_pdriver_op)->data_df_y_res));
	kp_himax_parse_assign_cmd(hx83102d_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
...
static void himax_hx83102d_func_re_init(void)
{
	...
	kp_g_core_fp->fp_suspend_ic_action = himax_hx83102d_suspend_ic_action;
...
#endif
#if defined(HX_ESD_RECOVERY)
	kp_g_core_fp->fp_0f_esd_check = hx83102d_0f_esd_check;
#endif
#endif
}
...
static void himax_hx83102e_reg_re_init(void)
{
	...
	kp_himax_parse_assign_cmd(hx83102e_data_df_tx,
			(*kp_pdriver_op)->data_df_tx,
			sizeof((*kp_pdriver_op)->data_df_tx));
	kp_himax_parse_assign_cmd(hx83102e_data_df_x_res,
			(*kp_pdriver_op)->data_df_x_res,
			sizeof((*kp_pdriver_op)->data_df_x_res));
	kp_himax_parse_assign_cmd(hx83102e_data_df_y_res,
			(*kp_pdriver_op)->data_df_y_res,
			sizeof((*kp_pdriver_op)->data_df_y_res));
