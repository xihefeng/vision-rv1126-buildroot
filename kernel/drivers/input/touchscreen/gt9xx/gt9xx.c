...
{
    GTP_DEBUG("Input ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
	if (gtp_change_x2y)
		GTP_SWAP(x, y);
    ...
    GTP_DEBUG("Output ID:%d, X:%d, Y:%d, W:%d", id, x, y, w);
}
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    if (ts->irq)
    {
        ...
        GTP_INFO("  <%s>_%d     ts->irq=%d irq_pin=%d  ret = %d\n", __func__, __LINE__, ts->irq, ts->irq_pin, ret);
    }else{
