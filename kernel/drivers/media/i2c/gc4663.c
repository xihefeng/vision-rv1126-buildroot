...
static int __gc4663_power_on(struct gc4663 *gc4663)
{
	...
	dev_info(dev, "gc4663 power on: Ok\n");
...
static void __gc4663_power_off(struct gc4663 *gc4663)
{
	...
	dev_info(dev, "gc4663 power Off: Ok\n");	
}
			...
			const struct i2c_device_id *id)
{
...
#ifdef CONFIG_VIDEO_V4L2_SUBDEV_API
	dev_info(dev, "CONFIG_VIDEO_V4L2_SUBDEV_API\n");
	sd->internal_ops = &gc4663_internal_ops;
	sd->flags |= V4L2_SUBDEV_FL_HAS_DEVNODE |
		     V4L2_SUBDEV_FL_HAS_EVENTS;
#endif
#if defined(CONFIG_MEDIA_CONTROLLER)
	dev_info(dev, "CONFIG_MEDIA_CONTROLLER\n");
	...
	dev_info(dev, "v4l2 async register subdev: %s\n", sd->name);
