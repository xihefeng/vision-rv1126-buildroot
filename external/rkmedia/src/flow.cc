...
int Flow::SetRunTimes(int _run_times) {
  ...
  RKMEDIA_LOGD("Flow:%s set run_times to %d\n", flow_tag.c_str(), run_times);
  return run_times;
}
...
void Flow::FlowMap::AddFlow(std::shared_ptr<Flow> flow, int index) {
  ...
  // Same down flow and same index is not allowed
...
void Flow::SendInput(std::shared_ptr<MediaBuffer> &input, int in_slot_index) {
  ...
  if (enable) {
    // fps control
    if (fps_out == 0) {
      RKMEDIA_LOGD("%s: %s: drop all buffer(fps_out=0)\n", GetFlowTag(),
                   __func__);
      return;
    ...
    if (drop_by_fps_ctrl) {
      RKMEDIA_LOGD("%s: %s: drop buffer by fps ctrl(%d --> %d)\n", GetFlowTag(),
                   __func__, fps_in, fps_out);
      return;
    }
