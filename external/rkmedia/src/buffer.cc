...
static int drm_device_open(const char *device = nullptr) {
  ...
  if (fd < 0) {
    RKMEDIA_LOGE(
        "Failed to open %s,please enable display_subsystem in the dts\n",
        device);
    return fd;
  }
...
static MediaBuffer alloc_drm_memory(size_t size, unsigned int flag,
                                    bool map = true) {
...
static MediaGroupBuffer *alloc_drm_memory_group(size_t size, unsigned int flag,
                                                bool map = true) {
  const static std::shared_ptr<DrmDevice> &drm_dev = DrmDevice::GetInstance();
  DrmBuffer *db = nullptr;
