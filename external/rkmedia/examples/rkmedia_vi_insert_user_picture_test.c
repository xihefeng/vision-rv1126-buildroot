...
int main(int argc, char *argv[]) {
  ...
  stUsrPicAttr.u32Width = 3840;
  stUsrPicAttr.u32Height = 2160;
  stUsrPicAttr.pvPicPtr = UsrPicArrary;
  ret = RK_MPI_VI_SetUserPic(0, &stUsrPicAttr);
  if (ret) {
    printf("ERROR: VI[0] set user pic error! ret=%d\n", ret);
    return 0;
  }
  ...
  printf("#Keep sleep 3s....\n");
  sleep(3);

  ret = RK_MPI_VI_EnableUserPic(0);
  if (ret) {
    printf("ERROR: VI[0] enable user pic error! ret=%d\n", ret);
    return 0;
  }
  printf("#Keep UserPic On 3s....\n");
  sleep(3);

  ret = RK_MPI_VI_DisableUserPic(0);
  if (ret) {
    printf("ERROR: VI[0] disable user pic error! ret=%d\n", ret);
    return 0;
  }
