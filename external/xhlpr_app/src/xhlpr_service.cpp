...
static void *DrawStream(void *data) {
	...
	while(!quit) {
		if(DrawGetFlag == 2) {
			...
			if (posixsm_share->plate_data_update == 1) {
				if (posixsm_share->count > 0) {
					img = new cv::Mat(cv::Size(session->EnVideoWidth, session->EnVideoHeight), CV_8UC3,
						(char *)RK_MPI_MB_GetPtr(DrawLastBuffer));
					for(int j = 0; j < posixsm_share->count;j++){
						cv::rectangle(*img, cv::Point(posixsm_share->tplate[j].points[0].x, posixsm_share->tplate[j].points[0].y-20), cv::Point(posixsm_share->tplate[j].points[0].x+80, posixsm_share->tplate[j].points[0].y), cv::Scalar(0, 0, 255), -1, 1, 0);
						cv::putText(*img, std::to_string(posixsm_share->tplate[j].points_score), cv::Point(posixsm_share->tplate[j].points[0].x, posixsm_share->tplate[j].points[0].y-4), 1, 1, CV_RGB(255, 255, 255));
						for(int k = 0;k < 4; k++){
							cv::line(*img,cv::Point(posixsm_share->tplate[j].points[k].x, posixsm_share->tplate[j].points[k].y), cv::Point(posixsm_share->tplate[j].points[(k+1)%4].x, posixsm_share->tplate[j].points[(k+1)%4].y), cv::Scalar(0, 0, 255), 4);
						}
					}
				}
				posixsm_share->count = 0;
				posixsm_share->plate_data_update = 0;
			}
...
int main(int argc, char *argv[]) {
	...
	snprintf(
		cmd, sizeof(cmd),
		"media-ctl -d %d -l '\"rkispp_input_image\":0->\"rkispp-subdev\":0[1]'",
		media);
	system(cmd);
	snprintf(cmd, sizeof(cmd),
		"media-ctl -d %d --set-v4l2 "
