		...
		if(DrawGetFlag == 2) {
			...
			if (posixsm_share->rockx_data_update == 1) {
				...
				for (int i = 0; i < posixsm_share->face_mask_array.count; i++) {
					if(posixsm_share->face_mask_array.face_masks[i].mask_score > 0.8) {
						cv::rectangle(*img, 
							cv::Point(posixsm_share->face_mask_array.face_masks[i].face_box.left, posixsm_share->face_mask_array.face_masks[i].face_box.top), 
							cv::Point(posixsm_share->face_mask_array.face_masks[i].face_box.right, posixsm_share->face_mask_array.face_masks[i].face_box.bottom), 
							cv::Scalar(0, 0, 255), 2);
						cv::putText(*img, std::to_string(posixsm_share->face_mask_array.face_masks[i].mask_score),
							cv::Point(posixsm_share->face_mask_array.face_masks[i].face_box.left, posixsm_share->face_mask_array.face_masks[i].face_box.top - 4),
							2, 1, cv::Scalar(255, 255, 255));
						if(posixsm_share->face_mask_array.face_masks[i].hasMask) {
							cv::putText(*img, id2class[0],
								cv::Point(posixsm_share->face_mask_array.face_masks[i].face_box.left, posixsm_share->face_mask_array.face_masks[i].face_box.top - 30),
								2, 1, cv::Scalar(255, 255, 255));
						
						} else {
							cv::putText(*img, id2class[1],
								cv::Point(posixsm_share->face_mask_array.face_masks[i].face_box.left, posixsm_share->face_mask_array.face_masks[i].face_box.top - 30),
								2, 1, cv::Scalar(255, 255, 255));
						}
					}
