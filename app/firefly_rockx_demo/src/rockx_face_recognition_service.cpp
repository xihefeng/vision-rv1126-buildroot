				...
				for (int i = 0; i < posixsm_share->face_array.count; i++) {
					if(posixsm_share->face_array.object[i].score > 0.8) {
						cv::rectangle(*img, 
							...
							cv::Scalar(0, 0, 255), 2);
					if(posixsm_share->similarity > 1.7) {
						cv::Size text_size = cv::getTextSize(posixsm_share->person_name, 20, 4, 5, &baseline);

						cv::Point origin; 
						origin.x = session->EnVideoWidth  / 2 - text_size.width / 2;
						origin.y = session->EnVideoHeight / 2 + text_size.height / 2 + 300;
						cv::rectangle(*img,
							cv::Point(session->EnVideoWidth  / 2 - text_size.width / 2 - 50, session->EnVideoHeight / 2 + text_size.height / 2 + 50 + 300),
							cv::Point(session->EnVideoWidth  / 2 + text_size.width / 2 + 50, session->EnVideoHeight / 2 - text_size.height / 2 - 50 + 300),
							cv::Scalar(225, 105, 65), -1);

						cv::putText(*img, posixsm_share->person_name,
							origin,
							20, 4, cv::Scalar(255, 255, 255), 5, 4);
					}
/*
