		...
		if(DrawGetFlag == 2) {
			...
			if (posixsm_share->rockx_data_update == 1) {
				...
				for (int i = 0; i < posixsm_share->face_array.count; i++) {
					...
					sprintf(angle_data, "angle=(pitch:%.2f yaw:%.2f roll:%.2f)",
						posixsm_share->out_angle.pitch,
						posixsm_share->out_angle.yaw,
						posixsm_share->out_angle.roll);
					cv::putText(*img, angle_data,
						cv::Point(posixsm_share->face_array.object[i].box.left, posixsm_share->face_array.object[i].box.top - 50),
						2, 1, cv::Scalar(255, 255, 255));
				}

        			for(int j = 0; j < posixsm_share->out_landmark.landmarks_count; j++) {
					cv::circle(*img, cv::Point(posixsm_share->out_landmark.landmarks[j].x, posixsm_share->out_landmark.landmarks[j].y), 5, {0, 255, 0}, -1);
				}
				memset(&posixsm_share->out_landmark, 0, sizeof(rockx_face_landmark_t));
				memset(&posixsm_share->out_angle, 0, sizeof(rockx_face_angle_t));
				memset(&posixsm_share->face_array, 0, sizeof(rockx_object_array_t));
				posixsm_share->out_landmark.landmarks_count = 0;
				posixsm_share->rockx_data_update = 0;
			}
