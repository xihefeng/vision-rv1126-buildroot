		...
		if(DrawGetFlag == 2) {
			if (posixsm_share->rockx_data_update == 1) {
				...
				for (int i = 0; i < posixsm_share->head_array.count; i++) {
					cv::rectangle(*img, 
						cv::Point(posixsm_share->head_array.object[i].box.left, posixsm_share->head_array.object[i].box.top), 
						cv::Point(posixsm_share->head_array.object[i].box.right, posixsm_share->head_array.object[i].box.bottom), 
						cv::Scalar(0, 0, 255), 2);
					cv::putText(*img, std::to_string(posixsm_share->head_array.object[i].score),
						cv::Point(posixsm_share->head_array.object[i].box.left, posixsm_share->head_array.object[i].box.top - 4),
