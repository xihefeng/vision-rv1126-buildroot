...
MEDIA_BUFFER PersonDetectionBuffer;
rockx_config_t rockx_configs;
volatile int PersonDetectionGetFlag = 0;
...
void PersonDetectionCb(MEDIA_BUFFER buffer)
{
	if(PersonDetectionGetFlag == 0) {
		PersonDetectionBuffer = RK_MPI_MB_Copy(buffer, RK_TRUE);
		PersonDetectionGetFlag = 1;
	}
...
static void *PersonDetectionStream(void *data) {
	...
	RK_MPI_SYS_RegisterOutCb(&stRgaChn0, PersonDetectionCb);
	...
	ret = rockx_create(&face_det_handle, ROCKX_MODULE_PERSON_DETECTION_V2, &rockx_configs, sizeof(rockx_config_t));
	...
	while (!quit) {
		if(PersonDetectionGetFlag == 1) {
			if (RK_MPI_MB_GetPtr(PersonDetectionBuffer) == NULL){
				PersonDetectionGetFlag = 0;
        			continue;
			}

			input_image.data = (uint8_t *)RK_MPI_MB_GetPtr(PersonDetectionBuffer);
			...
			if (posixsm_share->rockx_data_update == 0) {
				memcpy(&posixsm_share->person_array, &person_array, sizeof(rockx_object_array_t));
				// process result
				for (int i = 0; i < posixsm_share->person_array.count; i++) {
					posixsm_share->person_array.object[i].box.left *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->person_array.object[i].box.top *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
					posixsm_share->person_array.object[i].box.right *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->person_array.object[i].box.bottom *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
				}
				posixsm_share->rockx_data_update = 1;
			}
    			RK_MPI_MB_ReleaseBuffer(PersonDetectionBuffer);
    			PersonDetectionGetFlag = 0;
    		}
	}
...
{  
	...
	pthread_t person_detection_thread;
	pthread_create(&person_detection_thread, NULL, PersonDetectionStream, session_cfg);//车牌检测
	...
	pthread_join(person_detection_thread, NULL);
	return 0;
}  
