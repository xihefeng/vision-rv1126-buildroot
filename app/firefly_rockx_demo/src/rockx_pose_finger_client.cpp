...
MEDIA_BUFFER PoseFingerBuffer;
rockx_config_t rockx_configs;
volatile int PoseFingerGetFlag = 0;
...
void PoseFingerCb(MEDIA_BUFFER buffer)
{
	if(PoseFingerGetFlag == 0) {
		PoseFingerBuffer = RK_MPI_MB_Copy(buffer, RK_TRUE);
		PoseFingerGetFlag = 1;
	}
	RK_MPI_MB_ReleaseBuffer(buffer);
}

static void *PoseFingerStream(void *data) {
	...
	RK_MPI_SYS_RegisterOutCb(&stRgaChn0, PoseFingerCb);
	...
	ret = rockx_create(&pose_finger_handle, target_module, &rockx_configs, sizeof(rockx_config_t));
	...
	while (!quit) {
		if(PoseFingerGetFlag == 1) {
			if (RK_MPI_MB_GetPtr(PoseFingerBuffer) == NULL){
				PoseFingerGetFlag = 0;
        			continue;
			}

			input_image.data = (uint8_t *)RK_MPI_MB_GetPtr(PoseFingerBuffer);
			...
			if (posixsm_share->rockx_data_update == 0) {
				memcpy(&posixsm_share->finger, &finger, sizeof(rockx_keypoints_t));
				// process result
				if(posixsm_share->finger.count > 0) {
					for(int j = 0; j < finger.count; j++) {
						posixsm_share->finger.points[j].x *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
						posixsm_share->finger.points[j].y *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
					}
				}
				posixsm_share->rockx_data_update = 1;
			}

			RK_MPI_MB_ReleaseBuffer(PoseFingerBuffer);
			PoseFingerGetFlag = 0;
		}
	}
...
{  
	...
	pthread_t pose_finger_thread;
	pthread_create(&pose_finger_thread, NULL, PoseFingerStream, session_cfg);//车牌检测
	...
	pthread_join(pose_finger_thread, NULL);
	return 0;
}  
