	...
	while(!quit) {
		if(DrawGetFlag == 2) {
			...
			if (posixsm_share->rockx_data_update == 1) {
				if (posixsm_share->body_array.count > 0) {
					...
					for (int i = 0; i < posixsm_share->body_array.count; i++) {
						...
						for(int j = 0; j < posixsm_share->body_array.keypoints[i].count; j++) {
							int x = posixsm_share->body_array.keypoints[i].points[j].x;
							int y = posixsm_share->body_array.keypoints[i].points[j].y;
							float score = posixsm_share->body_array.keypoints[i].score[j];
						...
						for(int j = 0; j < posePairs.size(); j ++) {
							...
							int x0 = posixsm_share->body_array.keypoints[i].points[posePair.first].x;
							int y0 = posixsm_share->body_array.keypoints[i].points[posePair.first].y;
							int x1 = posixsm_share->body_array.keypoints[i].points[posePair.second].x;
							int y1 = posixsm_share->body_array.keypoints[i].points[posePair.second].y;
