		...
		if(DrawGetFlag == 2) {
			...
			if (posixsm_share->rockx_data_update == 1) {
				...
				for (int i = 0; i < posixsm_share->out_track_objects.count; i++) {
					cv::rectangle(*img, 
						cv::Point(posixsm_share->out_track_objects.object[i].box.left, posixsm_share->out_track_objects.object[i].box.top), 
						cv::Point(posixsm_share->out_track_objects.object[i].box.right, posixsm_share->out_track_objects.object[i].box.bottom), 
						cv::Scalar(0, 0, 255), 2);

					cv::putText(*img, ROCKX_OBJECT_DETECTION_LABELS_91[posixsm_share->out_track_objects.object[i].cls_idx],
						cv::Point(posixsm_share->out_track_objects.object[i].box.left, posixsm_share->out_track_objects.object[i].box.top - 4),
