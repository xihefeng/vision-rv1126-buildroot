...
MEDIA_BUFFER HeadDetectionBuffer;
rockx_config_t rockx_configs;
volatile int HeadDetectionGetFlag = 0;
...
void HeadDetectionCb(MEDIA_BUFFER buffer)
{
	if(HeadDetectionGetFlag == 0) {
		HeadDetectionBuffer = RK_MPI_MB_Copy(buffer, RK_TRUE);
		HeadDetectionGetFlag = 1;
	}
	RK_MPI_MB_ReleaseBuffer(buffer);
}

static void *HeadDetectionStream(void *data) {
	...
	RK_MPI_SYS_RegisterOutCb(&stRgaChn0, HeadDetectionCb);
	...
	rockx_module_t target_module = ROCKX_MODULE_HEAD_DETECTION;

	rockx_handle_t head_det_handle;
	ret = rockx_create(&head_det_handle, target_module, &rockx_configs, sizeof(rockx_config_t));
	...
	while (!quit) {
		if(HeadDetectionGetFlag == 1) {
			if (RK_MPI_MB_GetPtr(HeadDetectionBuffer) == NULL){
				HeadDetectionGetFlag = 0;
        			continue;
			}

			input_image.data = (uint8_t *)RK_MPI_MB_GetPtr(HeadDetectionBuffer);
			...
			if (posixsm_share->rockx_data_update == 0) {
				memcpy(&posixsm_share->head_array, &head_array, sizeof(rockx_keypoints_t));
				// process result
				for (int i = 0; i < posixsm_share->head_array.count; i++) {
					posixsm_share->head_array.object[i].box.left *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->head_array.object[i].box.top *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
					posixsm_share->head_array.object[i].box.right *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->head_array.object[i].box.bottom *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
				}
				posixsm_share->rockx_data_update = 1;
			}

			RK_MPI_MB_ReleaseBuffer(HeadDetectionBuffer);
			HeadDetectionGetFlag = 0;
		}
	}
...
{  
	...
	pthread_t head_detection_thread;
	pthread_create(&head_detection_thread, NULL, HeadDetectionStream, session_cfg);
	...
	pthread_join(head_detection_thread, NULL);
	return 0;
}  
