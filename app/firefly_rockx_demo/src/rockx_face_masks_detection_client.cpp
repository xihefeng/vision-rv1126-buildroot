...
MEDIA_BUFFER FaceMasksDetectionBuffer;
rockx_config_t rockx_configs;
volatile int FaceMasksDetectionGetFlag = 0;
...
void FaceMasksDetectionCb(MEDIA_BUFFER buffer)
{
	if(FaceMasksDetectionGetFlag == 0) {
		FaceMasksDetectionBuffer = RK_MPI_MB_Copy(buffer, RK_TRUE);
		FaceMasksDetectionGetFlag = 1;
	}
	RK_MPI_MB_ReleaseBuffer(buffer);
}

static void *FaceMasksDetectionStream(void *data) {
	...
	RK_MPI_SYS_RegisterOutCb(&stRgaChn0, FaceMasksDetectionCb);
	...
	ret = rockx_create(&face_masks_det_handle, ROCKX_MODULE_FACE_MASKS_DETECTION, &rockx_configs, sizeof(rockx_config_t));
	...
	while (!quit) {
		if(FaceMasksDetectionGetFlag == 1) {
			if (RK_MPI_MB_GetPtr(FaceMasksDetectionBuffer) == NULL){
				FaceMasksDetectionGetFlag = 0;
        			continue;
			}

			input_image.data = (uint8_t *)RK_MPI_MB_GetPtr(FaceMasksDetectionBuffer);
			...
			if (posixsm_share->rockx_data_update == 0) {
				memcpy(&posixsm_share->face_mask_array, &face_mask_array, sizeof(rockx_face_mask_array_t));
				// process result
				for (int i = 0; i < posixsm_share->face_mask_array.count; i++) {
					posixsm_share->face_mask_array.face_masks[i].face_box.left *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->face_mask_array.face_masks[i].face_box.top *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
					posixsm_share->face_mask_array.face_masks[i].face_box.right *= (float)((float)session->EnVideoWidth / session->OutVideoWidth);
					posixsm_share->face_mask_array.face_masks[i].face_box.bottom *= (float)((float)session->EnVideoHeight / session->OutVideoHeight);
				}
				posixsm_share->rockx_data_update = 1;
			}
    			RK_MPI_MB_ReleaseBuffer(FaceMasksDetectionBuffer);
    			FaceMasksDetectionGetFlag = 0;
    		}
	}
...
{  
	...
	pthread_t face_masks_detection_thread;
	pthread_create(&face_masks_detection_thread, NULL, FaceMasksDetectionStream, session_cfg);//车牌检测
	...
	pthread_join(face_masks_detection_thread, NULL);
	return 0;
}  
