...
void camera_control(unsigned char cs, unsigned int val) {
  ...
  case UVC_PU_BRIGHTNESS_CONTROL:
    SAMPLE_COMM_ISP_SET_Brightness(s32CamId_0, val);
    SAMPLE_COMM_ISP_SET_Brightness(s32CamId_1, val);
    break;
  case UVC_PU_CONTRAST_CONTROL:
    SAMPLE_COMM_ISP_SET_Contrast(s32CamId_0, val);
    SAMPLE_COMM_ISP_SET_Contrast(s32CamId_1, val);
    break;
  case UVC_PU_HUE_CONTROL:
    break;
  case UVC_PU_SATURATION_CONTROL:
    SAMPLE_COMM_ISP_SET_Saturation(s32CamId_0, val);
    SAMPLE_COMM_ISP_SET_Saturation(s32CamId_1, val);
    break;
  case UVC_PU_SHARPNESS_CONTROL:
    SAMPLE_COMM_ISP_SET_Sharpness(s32CamId_0, val);
    SAMPLE_COMM_ISP_SET_Sharpness(s32CamId_1, val);
    break;
...
*/
/*
  ret = SAMPLE_COMM_ISP_Init(s32CamId_0, RK_AIQ_WORKING_MODE_NORMAL, RK_TRUE, pIqfilesPath);
  if (ret)
    return -1;
  SAMPLE_COMM_ISP_Run(s32CamId_0);
  SAMPLE_COMM_ISP_SetFrameRate(s32CamId_0, fps);
*/
  ret = SAMPLE_COMM_ISP_Init(s32CamId_1, RK_AIQ_WORKING_MODE_NORMAL, RK_TRUE, pIqfilesPath);
  if (ret)
    return -1;
  SAMPLE_COMM_ISP_Run(s32CamId_1);
  SAMPLE_COMM_ISP_SetFrameRate(s32CamId_1, fps);
  ...
  for (int i = 0; i < cfg.session_count; i++) {
    // VI create
    printf("VI[%d] create\n", i);
    SAMPLE_COMMON_VI_Start(&cfg.session_cfg[i], VI_WORK_MODE_NORMAL, s32CamId_1, width, height, i);
    if (i == DRAW_INDEX)
      RK_MPI_VI_StartStream(s32CamId_1, cfg.session_cfg[DRAW_INDEX].stViChn.s32ChnId);
  }
...
void camera_stop(void) {
  if (start) {
    ...
    for (int i = 0; i < cfg.session_count; i++) {
      ...
      RK_MPI_VI_DisableChn(s32CamId_1, cfg.session_cfg[i].stViChn.s32ChnId);
    }
#ifdef RKAIQ
    if (pIqfilesPath) {
      register_uvc_pu_control_callback(NULL);
      //SAMPLE_COMM_ISP_Stop(s32CamId_0);
      SAMPLE_COMM_ISP_Stop(s32CamId_1);
    }
#endif
  }
}
