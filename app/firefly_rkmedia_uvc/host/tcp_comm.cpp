...
tcp_comm::tcp_comm()
{
	...
	tcps_saddr.sin_addr.s_addr = htonl(INADDR_ANY);// INADDR_ANY 表示 0.0.0.0地址
}

int tcp_comm::tcpc_connect(const char *ip,int port)
{

	tcpc_saddr.sin_family = AF_INET; //地址族	
	tcpc_saddr.sin_port = htons(port); //端口，需要使用网络数据格式，这里htons()函数就是用来用作格式转换
	tcpc_saddr.sin_addr.s_addr = inet_addr(ip);// IP inet_addr 转换格式

	if ((client_sockfd = socket(AF_INET, SOCK_STREAM, 0)) == -1) { //申请 sock 套节字，SOCK_STREAM 代表通信类型是 TCP
	...
	if (connect(client_sockfd, (struct sockaddr *)&tcpc_saddr, sizeof(struct sockaddr_in)) == -1) { // connect tcpc_saddr 设置的 IP 地址
