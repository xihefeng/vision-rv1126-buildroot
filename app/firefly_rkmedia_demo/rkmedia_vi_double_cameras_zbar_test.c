...
static void *process(void *arg)
{
  ...
  while (!quit)
  {
    ...
    if(g_snap) {
      ...
      zbar_qrcode_scan(buff, size, g_width, g_height);
    }
    if(quit){
    	break;
    }
...
int main(int argc, char *argv[])
{
  ...
  SAMPLE_COMM_ISP_SetFrameRate(1, fps);
  SAMPLE_COMM_ISP_SET_mirror(1, 1);
#else
  (void)argc;
  (void)argv;
#endif
  ...
  stVoAttr.stDispRect.u32Width = 800;
  stVoAttr.stDispRect.u32Height = 1280;
