...
struct main_stream_data {
	int number;
};

// SSD
static char *g_box_priors = (char*)"/oem/usr/share/rknn_model/box_priors.txt";
static char *g_labels_list = (char*)"/oem/usr/share/rknn_model/coco_labels_list.txt";
static char *g_ssd_path =
    (char*)"/oem/usr/share/rknn_model/ssd_inception_v2_rv1109_rv1126.rknn";
...
int postProcessSSD(float *predictions, float *output_classes, int width,
  ...
  for (int i = 0; i < validCount; ++i) {
	...
	group->results[last_count].box.topclass = topClassScoreIndex;
...
struct Session {
  ...
  MPP_CHN_S stVdecChn;
  MPP_CHN_S stVenChn;
  MPP_CHN_S stRgaVoChn;
  MPP_CHN_S stRgaNNChn;
  MPP_CHN_S stRgaVENChn;
  MPP_CHN_S stRgaDRAWChn;
};
...
int FFRKMedia_Vdec_Send(u_int8_t* framebuff,unsigned framesize,bool * quit,int cur_chn)
{
...
static void *GetMediaBuffer(void *data) {
  ...
  while (g_flag_run) {
    buffer = RK_MPI_SYS_GetMediaBuffer(RK_ID_RGA, cfg.session_cfg[nndata->number].stRgaNNChn.s32ChnId, -1); //取 RK_NN_INDEX 通道的帧进行推理，因为
...
*/
    ...
    // Post Process
    detect_result_group_t detect_result_group; //解析推理结果，得出物体类别位置信息
    ...
    if (detect_result_group.count > 0) {
      rknn_list_push(rknn_list_[nndata->number], getCurrentTimeMsec(), detect_result_group);//把信息存放到链表中去
      int size = rknn_list_size(rknn_list_[nndata->number]);
      if (size >= MAX_RKNN_LIST_NUM)
        rknn_list_drop(rknn_list_[nndata->number]);
      // printf("size is %d\n", size);
    }
...
static void *MainStream(void * data)  {
  MEDIA_BUFFER buffer;
  main_stream_data * main_data = (struct main_stream_data *)data;
  float x_rate = (float)cfg.session_cfg[main_data->number].u32Width / MODEL_INPUT_SIZE; //计算推理图片和rtsp 源图片的坐标比例，来复原识别物体真实位置
  float y_rate =
      (float)cfg.session_cfg[main_data->number].u32Height / MODEL_INPUT_SIZE;
  cv::Mat * dst = NULL;
  printf("x_rate is %f, y_rate is %f\n", x_rate, y_rate);

  while (g_flag_run) {
    buffer = RK_MPI_SYS_GetMediaBuffer(
        RK_ID_RGA, cfg.session_cfg[main_data->number].stRgaVENChn.s32ChnId, -1);
    if (!buffer)
      continue;
    // draw
    if (rknn_list_size(rknn_list_[main_data->number])) {
      long time_before;
      detect_result_group_t detect_result_group;
      memset(&detect_result_group, 0, sizeof(detect_result_group));
      rknn_list_pop(rknn_list_[main_data->number], &time_before, &detect_result_group);
      // printf("time interval is %ld\n", getCurrentTimeMsec() - time_before); //因为是异步的，这里可以调试查看识别延时

      dst = new cv::Mat(cv::Size(cfg.session_cfg[main_data->number].u32Width,
			cfg.session_cfg[main_data->number].u32Height), CV_8UC3,
			(char *)RK_MPI_MB_GetPtr(buffer));
	...
	//  continue;
        ...
        if (y < 0)
          y = 0;
        while ((uint32_t)(x + w) >= cfg.session_cfg[main_data->number].u32Width) {
          w -= 16;
        }
        while ((uint32_t)(y + h) >= cfg.session_cfg[main_data->number].u32Height) {
          h -= 16;
        }
	if (debug)
...
*/
	...
	cv::rectangle(*dst, p1, p2,
			colorArray[detect_result_group.results[j].box.topclass %
			10], 2);
	cv::putText(*dst, detect_result_group.results[j].name, p_name, 1, 2,
			colorArray[detect_result_group.results[j].box.topclass %
			10], 2);
      }
      delete dst;
    }
    // send from Vdec to VENC
    RK_MPI_SYS_SendMediaBuffer(
        RK_ID_RGA, cfg.session_cfg[main_data->number].stRgaDRAWChn.s32ChnId, buffer); //换框后发送本帧数据到 RgaDRAW，RGA转换格式，编码然后送往 RTSP 流。
    
    // send from Vdec to VO
    if (vo_enable) {
    	RK_MPI_SYS_SendMediaBuffer(
        RK_ID_RGA, cfg.session_cfg[main_data->number].stRgaVoChn.s32ChnId, buffer); //画框后发送本帧数据到 VO。
    }
    RK_MPI_MB_ReleaseBuffer(buffer);
  }
...
static void dump_cfg() {
  for (int i = 0; i < cfg.session_count; i++) {
    ...
    printf("video_fps = %d.\n", cfg.session_cfg[i].video_fps);
    printf("width = %d.\n", cfg.session_cfg[i].u32Width);
    printf("height = %d.\n", cfg.session_cfg[i].u32Height);
    printf("video url =%s.\n", cfg.session_cfg[i].videourl);
    printf("port =%d.\n", cfg.session_cfg[i].port);
...
static int load_cfg(const char *cfg_file) {
  ...
  while (fgets(line, sizeof(line) - 1, fp)) {
    ...
    if ((p = strstr(line, "video_type="))) {
      if (sscanf(p,
                 "video_type=%d video_fps=%d width=%u height=%u image_type=%u port=%d video_url=%s",
                 &cfg.session_cfg[count].video_type,
                 &cfg.session_cfg[count].video_fps,
                 &cfg.session_cfg[count].u32Width,
                 &cfg.session_cfg[count].u32Height,
                 &cfg.session_cfg[count].enImageType,
                 &cfg.session_cfg[count].port,
                 cfg.session_cfg[count].videourl) == 0) {
...
static void SAMPLE_COMMON_RGA_Start(struct Session *session) {
  int ret = 0;
  ...
  stRgaAttr.stImgIn.imgType = session->enImageType;
  stRgaAttr.stImgIn.u32Width = session->u32Width;
  stRgaAttr.stImgIn.u32Height = session->u32Height;
  ...
  stRgaAttr.stImgOut.u32Width = session->u32Width;
  stRgaAttr.stImgOut.u32Height = session->u32Height;
  stRgaAttr.stImgOut.u32HorStride = session->u32Width;
  stRgaAttr.stImgOut.u32VirStride = CELING_2_POWER(session->u32Height,16);
  ret = RK_MPI_RGA_CreateChn(session->stRgaVENChn.s32ChnId, &stRgaAttr);
  if (ret) {
    printf("ERROR: Create stRgaVENChn[%d] falied! ret=%d\n", session->stRgaVENChn.s32ChnId, ret);
  }
  ...
  ret = RK_MPI_RGA_CreateChn(session->stRgaNNChn.s32ChnId, &stRgaAttr);
  if (ret) {
    printf("ERROR: Create stRgaNNChn[%d] falied! ret=%d\n", session->stRgaNNChn.s32ChnId, ret);
  }


  stRgaAttr.u16Rotaion = 90;
  stRgaAttr.stImgOut.imgType = session->enImageType;
  stRgaAttr.stImgOut.u32Width = session->u32Height;
  stRgaAttr.stImgOut.u32Height = session->u32Width;
  stRgaAttr.stImgOut.u32HorStride = CELING_2_POWER(session->u32Height,16);
  stRgaAttr.stImgOut.u32VirStride = session->u32Width;
  
  if (vo_enable) {
	  ret = RK_MPI_RGA_CreateChn(session->stRgaVoChn.s32ChnId, &stRgaAttr);
	  if (ret) {
		  printf("ERROR: Create stRgaVoChn[%d] falied! ret=%d\n", session->stRgaVoChn.s32ChnId, ret);
	  }
  }
  stRgaAttr.u16Rotaion = 0;
  ...
  stRgaAttr.stImgOut.u32Height = session->u32Height;
  stRgaAttr.stImgOut.u32HorStride = session->u32Width;
  stRgaAttr.stImgOut.u32VirStride = CELING_2_POWER(session->u32Height,16);//16对齐
  ret = RK_MPI_RGA_CreateChn(session->stRgaDRAWChn.s32ChnId, &stRgaAttr);
  if (ret) {
    printf("ERROR: Create stRgaDRAWChn[%d] falied! ret=%d\n", session->stRgaDRAWChn.s32ChnId, ret);
  }
}

static void SAMPLE_COMMON_VDEC_Start(struct Session *session) {
  int ret = 0;
  VDEC_CHN_ATTR_S stVdecAttr;
  stVdecAttr.enCodecType = session->video_type; //解码格式
  stVdecAttr.enMode = VIDEO_MODE_STREAM;//流
  stVdecAttr.enDecodecMode = VIDEO_DECODEC_HADRWARE;
  
  ret = RK_MPI_VDEC_CreateChn(session->stVdecChn.s32ChnId, &stVdecAttr);//创建解码器通道 
  if (ret) {
    printf("Create stVdecChn[%d] failed! ret=%d\n", session->stVdecChn.s32ChnId, ret);
  }
}

static void SAMPLE_COMMON_VENC_Start(struct Session *session) {
  VENC_CHN_ATTR_S venc_chn_attr;
  int fps = session->video_fps;
  ...
  venc_chn_attr.stVencAttr.u32VirHeight = CELING_2_POWER(session->u32Height,16);
  ...
  case RK_CODEC_TYPE_H264:
    ...
    venc_chn_attr.stRcAttr.stH264Cbr.u32BitRate =
        session->u32Width * session->u32Height * fps / 14;
...
static RK_CHAR optstr[] = "?::a::c:b:l:p:M:d:v";
static const struct option long_options[] = {
    ...
    {"output_Vo", required_argument, NULL, 'v'},
...
static void print_usage(const RK_CHAR *name) {
  ...
  printf("\t-v | --Vo: Output to Vo\n");
  printf("\trstp: rtsp://<ip>/tmp/H26X_stream\n");
}
...
int main(int argc, char **argv) {
  int model_len = 0;
  struct rv1126_data nndata[MAXFFRTSPChn];
  struct main_stream_data main_data[MAXFFRTSPChn];
  unsigned char *model;
  RK_CHAR *pCfgPath = (RK_CHAR*)"/oem/usr/share/rtsp-nn.cfg";
  ...
  while ((c = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
    ...
    case 'a':
      ...
      if (tmp_optarg) {
        ...
        pIqfilesPath = (RK_CHAR*)"/oem/etc/iqfiles/";
      }
      break;
    case 'c':
      pCfgPath = optarg;
  ...
  load_cfg(pCfgPath);//解析 rtsp-nn.cfg file
  ...
  //大循环用的变量是j
  //小循环用的变量是i
  for (int j = 0; j < cfg.session_count; j++) {
	printf("Loading model ...\n");
	model = load_model(g_ssd_path, &model_len);
	ret = rknn_init(&nndata[j].ctx, model, model_len, 0);
	if (ret < 0) {
	...
	ret = rknn_query(nndata[j].ctx, RKNN_QUERY_IN_OUT_NUM, &nndata[j].io_num, sizeof(nndata[j].io_num));
	if (ret != RKNN_SUCC) {
		printf("rknn_query fail! ret=%d\n", ret);
		return -1;
	}
	printf("model input num: %d, output num: %d\n", nndata[j].io_num.n_input,
			nndata[j].io_num.n_output);

	printf("input tensors:\n");
	rknn_tensor_attr input_attrs[nndata[j].io_num.n_input];
	memset(input_attrs, 0, sizeof(input_attrs));

	for (unsigned int i = 0; i < nndata[j].io_num.n_input; i++) {
		input_attrs[i].index = i;
		ret = rknn_query(nndata[i].ctx, RKNN_QUERY_INPUT_ATTR, &(input_attrs[i]),
	...
	rknn_tensor_attr output_attrs[nndata[j].io_num.n_output];
	memset(output_attrs, 0, sizeof(output_attrs));
	for (unsigned int i = 0; i < nndata[j].io_num.n_output; i++) {
		output_attrs[i].index = i;
		ret = rknn_query(nndata[j].ctx, RKNN_QUERY_OUTPUT_ATTR, &(output_attrs[i]),
			             sizeof(rknn_tensor_attr));
		if (ret != RKNN_SUCC) {
		printf("rknn_query fail! ret=%d\n", ret);
		return -1;
  ...
  for (int i = 0; i < cfg.session_count; i++) {
    ...
    // VDec create
    printf("VDEC create\n");
    cfg.session_cfg[i].stVdecChn.enModId = RK_ID_VDEC;
    cfg.session_cfg[i].stVdecChn.s32ChnId = i;
    SAMPLE_COMMON_VDEC_Start(&cfg.session_cfg[i]);
    // Rga create
    printf("Rga create\n");
    cfg.session_cfg[i].stRgaVENChn.enModId = RK_ID_RGA;
    cfg.session_cfg[i].stRgaVENChn.s32ChnId = i * 4;
    cfg.session_cfg[i].stRgaNNChn.enModId = RK_ID_RGA;
    cfg.session_cfg[i].stRgaNNChn.s32ChnId = i * 4 + 1;
    cfg.session_cfg[i].stRgaVoChn.enModId = RK_ID_RGA;
    cfg.session_cfg[i].stRgaVoChn.s32ChnId = i * 4 + 2;
    cfg.session_cfg[i].stRgaDRAWChn.enModId = RK_ID_RGA;
    cfg.session_cfg[i].stRgaDRAWChn.s32ChnId = i * 4 + 3;
    SAMPLE_COMMON_RGA_Start(&cfg.session_cfg[i]);
...
/*
    ret = RK_MPI_SYS_RegisterOutCb(&cfg.session_cfg[i].stRgaVENChn, video_packet_cb);
    if (ret) {
		printf("ERROR: register output callback for VENC[0] error! ret=%d\n", ret);
		return 0;
    }
*/
    // VDec Bind Rga for RKNN
    ret = RK_MPI_SYS_Bind(&cfg.session_cfg[i].stVdecChn, &cfg.session_cfg[i].stRgaVENChn);
    if (ret) {
       printf("ERROR: Bind Vdec[%d] and stRgaVENChn[%d] failed! ret=%d\n",
			   cfg.session_cfg[i].stVdecChn.s32ChnId, cfg.session_cfg[i].stRgaVENChn.s32ChnId, ret);
       return -1;
    }
    ret = RK_MPI_SYS_Bind(&cfg.session_cfg[i].stVdecChn,&cfg.session_cfg[i].stRgaNNChn);
    if (ret) {
       printf("ERROR: Bind Vdec[%d] and stRgaNNChn[%d] failed! ret=%d\n",
			   cfg.session_cfg[i].stVdecChn.s32ChnId, cfg.session_cfg[i].stRgaNNChn.s32ChnId, ret);
       return -1;
    }
    
    ret = RK_MPI_SYS_Bind(&cfg.session_cfg[i].stRgaDRAWChn, &cfg.session_cfg[i].stVenChn);
    if (ret) {
       printf("ERROR: Bind RgaDRAW[%d] and stVenChn[%d] failed! ret=%d\n",
			   cfg.session_cfg[i].stRgaDRAWChn.s32ChnId, cfg.session_cfg[i].stVenChn.s32ChnId, ret);
       return -1;
    }
    // RTSP
    ffrtsp_get.count += 1;
    ffrtsp_get.ffrtsp_get_info[i].url = cfg.session_cfg[i].videourl;
    ffrtsp_push[i].idex = i;
    ffrtsp_push[i].port = cfg.session_cfg[i].port;
    ffrtsp_push[i].type = cfg.session_cfg[i].video_type;

    create_rknn_list(&rknn_list_[i]);
    printf("create rknn_list[%d] success\n", i);
  }
  ...
  stVoAttr.enImgType = cfg.session_cfg[0].enImageType;
  stVoAttr.u16Zpos = 1;
  stVoAttr.u32Width = SCREEN_WIDTH;
  stVoAttr.u32Height = SCREEN_HEIGHT;
  stVoAttr.stImgRect.s32X = 0;
  stVoAttr.stImgRect.s32Y = 0;
  stVoAttr.stImgRect.u32Width = cfg.session_cfg[0].u32Height;
  stVoAttr.stImgRect.u32Height = cfg.session_cfg[0].u32Width;
  stVoAttr.stDispRect.s32X = 0;
  stVoAttr.stDispRect.s32Y = 0;
  stVoAttr.stDispRect.u32Width = 265;
  stVoAttr.stDispRect.u32Height = 425;
  if(vo_enable) {
	...
	ret = RK_MPI_SYS_Bind(&cfg.session_cfg[0].stRgaVoChn,&VoChn);
	if (ret) {
  ...
  // Get RTSP Stream & Send to Vdec
  pthread_t rtsppush_thread[MAXFFRTSPChn];
  pthread_t read_thread[MAXFFRTSPChn];
  pthread_t main_stream_thread[MAXFFRTSPChn];

  for (int i = 0; i < cfg.session_count; i++) {
	main_data[i].number = i;
	nndata[i].number = i;
	printf("ffrtsp_get.ffrtsp_get_info[i].url = cfg.session_cfg[i].videourl = %s\n", ffrtsp_get.ffrtsp_get_info[i].url);
	//送入 NPU 推理得出最终物体类别坐标位置存放到链表中
	// Push RTSP Stream
	pthread_create(&rtsppush_thread[i], NULL, rtsppushbuff, (void *)&ffrtsp_push[i]);
	// RTSP 推流
	// Get the sub-stream buffer for humanoid recognition
	pthread_create(&read_thread[i], NULL, GetMediaBuffer, &nndata[i]); //线程内容：取 1 通道的 VI 视频源数据进行缩放
	//送入 NPU 推理得出最终物体类别坐标位置存放到链表中
	// The mainstream draws a box asynchronously based on the recognition result
	pthread_create(&main_stream_thread[i], NULL, MainStream, &main_data[i]);//线程内容：通过 0 通道获取主分辨图像，进行
	//绘框，最后送给编码器编码
	usleep(1000 * 1000);
  }

  pthread_t rtspget_thread;
  pthread_create(&rtspget_thread, NULL, rtspgetbuff, (void *)&ffrtsp_get); //线程内容：取 1 通道的 VI 视频源数据进行缩放

  while (g_flag_run) {
	  for (int i = 0; i < cfg.session_count; i++) {
		  if(ffrtsp_push[i].fp == NULL){
			  continue;
		  }
		  MEDIA_BUFFER buffer;
		  // send video buffer
		  ...
		  if (RK_MPI_MB_GetPtr(buffer) != NULL) {
			  fwrite(RK_MPI_MB_GetPtr(buffer), RK_MPI_MB_GetSize(buffer), 1, ffrtsp_push[i].fp);
			  RK_MPI_MB_ReleaseBuffer(buffer);
		  }
	  }
  }
  ...
  for (int i = 0; i < cfg.session_count; i++) {
	pthread_join(rtsppush_thread[i], NULL);
	pthread_join(read_thread[i], NULL);
	pthread_join(main_stream_thread[i], NULL);
    if (i != DRAW_INDEX)
      RK_MPI_SYS_UnBind(&cfg.session_cfg[i].stVdecChn,
                        &cfg.session_cfg[i].stVenChn);
	if (vo_enable)
		RK_MPI_SYS_UnBind(&cfg.session_cfg[i].stRgaVoChn,&VoChn);
	RK_MPI_SYS_UnBind(&cfg.session_cfg[i].stVdecChn,&cfg.session_cfg[i].stRgaVENChn);
	RK_MPI_SYS_UnBind(&cfg.session_cfg[i].stVdecChn,&cfg.session_cfg[i].stRgaNNChn);
	RK_MPI_SYS_UnBind(&cfg.session_cfg[i].stRgaDRAWChn, &cfg.session_cfg[i].stVenChn);

	RK_MPI_VENC_DestroyChn(cfg.session_cfg[i].stVenChn.s32ChnId);
	RK_MPI_RGA_DestroyChn(cfg.session_cfg[i].stRgaVENChn.s32ChnId);
	RK_MPI_RGA_DestroyChn(cfg.session_cfg[i].stRgaNNChn.s32ChnId);
	RK_MPI_RGA_DestroyChn(cfg.session_cfg[i].stRgaDRAWChn.s32ChnId);
	RK_MPI_VDEC_DestroyChn(cfg.session_cfg[i].stVdecChn.s32ChnId);
	if (vo_enable)
		RK_MPI_VO_DestroyChn(cfg.session_cfg[i].stRgaVoChn.s32ChnId);
	if (nndata[i].ctx)
		rknn_destroy(nndata[i].ctx);
	destory_rknn_list(&rknn_list_[i]);
  }
  pthread_join(rtspget_thread, NULL);
  if (model)
    free(model);
