...
bool parse_mcu_rkraws(const char *subdev, struct mcu_rkaiq_rkraw *mcu_rkraws) {
  ...
  printf("%s: resmem.resmem_padr 0x%x, size:0x%x\n", __FUNCTION__,
         resmem.resmem_padr, resmem.resmem_size);
  uint8_t *mem_vir_addr = (uint8_t *)mmap(NULL, resmem.resmem_size, PROT_WRITE,
                                          MAP_SHARED, mfd, resmem.resmem_padr);
  ...
  while (icnt < MAX_FAKE_FRAMES_NUM) {
    ...
    case 0xff01: {
      ...
      printf("==0x%x, 0x%x, 0x%x, 0x%x,0x%x\n", raw_data_ptr->flag,
             raw_data_ptr->size, raw_data_ptr->ddr_laddr,
             raw_data_ptr->ddr_haddr, raw_data_ptr->raw_size);
...
void alloc_rkraws(uint8_t **rkraws) {
  for (int i = 0; i < MAX_FAKE_FRAMES_NUM; i++) {
    rkraws[i] = (unsigned char *)malloc(2048);
  }
}
