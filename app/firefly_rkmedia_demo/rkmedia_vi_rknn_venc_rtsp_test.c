...
static void *GetMediaBuffer(void *arg) {
  ...
  while (g_flag_run) {
    buffer = RK_MPI_SYS_GetMediaBuffer(RK_ID_VI, RK_NN_INDEX, -1); //取 RK_NN_INDEX 通道的帧进行推理，因为
//这个通道本身分辨率很低，所以进行缩放的时候会省去 rga 的资源
    ...
    if (detect_result_group.count > 0) {
      rknn_list_push(rknn_list_, getCurrentTimeMsec(), detect_result_group);//把信息存放到链表中去
...
static void *MainStream() {
  ...
  float x_rate = (float)cfg.session_cfg[DRAW_INDEX].u32Width / MODEL_INPUT_SIZE; //计算推理图片和rtsp 源图片的坐标比例，来复原识别物体真实位置
  float y_rate =
  ...
  while (g_flag_run) {
    ...
    RK_MPI_SYS_SendMediaBuffer(
        RK_ID_VENC, cfg.session_cfg[DRAW_INDEX].stVenChn.s32ChnId, buffer); //换框后发送本帧数据到 VENC，编码然后送往 RTSP 流。
    RK_MPI_MB_ReleaseBuffer(buffer);
  }
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < cfg.session_count; i++) {
    ...
    if (i == DRAW_INDEX) //当主码流通道 0 是，打开不绑定 VENC 等待图像绘制使用
      RK_MPI_VI_StartStream(s32CamId, cfg.session_cfg[i].stViChn.s32ChnId);
    else //这里是子码流，由于不用图像处理直接绑定 VENC。子码流的分辨率低但是高于 RKNN 所需的 300x300 所以
	 //RKNN 图像推理将会从子码流取图像以节省 RGA 资源。
      RK_MPI_SYS_Bind(&cfg.session_cfg[i].stViChn,
                      &cfg.session_cfg[i].stVenChn);

    // rtsp video
    printf("rtsp video\n");
    switch (cfg.session_cfg[i].video_type) { //根据编码类型创建 rtsp 推流通道
    case RK_CODEC_TYPE_H264:
  ...
  pthread_create(&read_thread, NULL, GetMediaBuffer, NULL); //线程内容：取 1 通道的 VI 视频源数据进行缩放
  ...
  pthread_create(&main_stream_thread, NULL, MainStream, NULL);//线程内容：通过 0 通道获取主分辨图像，进行
  //绘框，最后送给编码器编码
